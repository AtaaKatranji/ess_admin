    ...
    <div className="flex flex-col md:flex-row h-screen bg-gray-100">
      ...
      <aside className={`${isSidebarOpen ? 'translate-x-0 ' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out fixed md:static top-0 left-0 z-40 w-64 h-screen bg-gray-800 text-white flex flex-col justify-between`}
        ...
        <nav className="p-4 space-y-4">
          ...
          <Link href={`/dashboard/institution/${slug}/employees/(employee)/${employeeId}/details`} className={`flex items-center py-2 px-4 ${isActive('details') ? 'bg-blue-600 rounded' : ''}`}>
            <ListCollapse className="mr-2 h-5 w-5" />
            Information
          </Link>
          <Link href={`/dashboard/institution/${slug}/employees/(employee)/${employeeId}/tasks`} className={`flex items-center py-2 px-4 ${isActive('tasks') ? 'bg-blue-600 rounded' : ''}`}>
