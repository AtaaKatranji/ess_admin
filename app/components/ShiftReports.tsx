...
const generateLastMonths = (lang: string, count = 6) => {
  const months = [];
  const today = new Date();
  for (let i = 0; i < count; i++) {
    const date = new Date(today.getFullYear(), today.getMonth() - i, 1);
    const value = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`; // "YYYY-MM"
    const label = date.toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", { year: 'numeric', month: 'long' }); // Localized
...
export default function ShiftReport({ open, onOpenChange, shiftId, institutionKey }: ShiftReportProps) {
  ...
  const months = generateLastMonths(lang);
  ...
  useEffect(() => {
    ...
    shiftAPI.fetchShiftReport(selectedShift, selectedMonth, institutionKey)
      ...
      .catch((err) => {
        setShiftData(null);
        console.error(err);
        setError(err.message || t("reports.noData"));
      })
    ...
    fetchAttendanceIssues(selectedShift, selectedMonth)
      ...
      .catch((err) => {
        setAttendanceIssues([]);
        console.error(err);
        setIssuesError(err.message || t("reports.loadingIssues"));
      })
      .finally(() => setIssuesLoading(false));

  }, [selectedShift, selectedMonth, institutionKey, t]);
  ...
  const getIssueLabel = (issueType: AttendanceIssueUI["issueType"]) => {
    switch (issueType) {
      case "MISSING_CHECKOUT":
        return t("reports.issue.missingCheckout")
      case "SHORT_DAY":
        return t("reports.issue.shortDay")
    }
  }
  ...
  const getStatusBadge = (attendanceRate: number) => {
    if (attendanceRate >= 90) return <Badge className="bg-green-100 text-green-800">{t("reports.status.excellent")}</Badge>
    if (attendanceRate >= 75) return <Badge className="bg-blue-100 text-blue-800">{t("reports.status.good")}</Badge>
    if (attendanceRate >= 60) return <Badge className="bg-yellow-100 text-yellow-800">{t("reports.status.fair")}</Badge>
    return <Badge className="bg-red-100 text-red-800">{t("reports.status.poor")}</Badge>
  }
  ...
  function formatShiftTimes(shiftTimes: ShiftTimes) {
    ...
    for (const day of daysOrder) {
      ...
      if (shift) {
        ...
        timeGroups[time].push(t(`days.${day}`) + ',');
      }
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <Select value={selectedMonth} onValueChange={setSelectedMonth}>
            <SelectTrigger className="w-[180px]" aria-label={t("reports.selectMonth")}>
          ...
          <Select value={selectedShift} onValueChange={setSelectedShift}>
            <SelectTrigger className="w-[180px]" aria-label={t("reports.selectShift")}>
              <SelectValue placeholder={t("reports.selectShift")} />
            </SelectTrigger>
            <SelectContent>
              {shifts.map((shift) => (
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900"><span className="text-sm text-gray-800">{t("reports.shift")}</span> {shiftData?.shiftName}</h1>
                <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                  <div className="flex items-center gap-1">
                    <Calendar className="w-4 h-4" />
                    <span>{t("reports.month")} {shiftData?.monthName}</span>
                  </div>
                ...
                <Card key={metric.label} className={`${iconBg} p-4`}>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium ">
                      {t(`reports.table.${metric.label.toLowerCase().replace(/ /g, "")}`) || metric.label}
                    </CardTitle>
          ...
          {issuesLoading && <p>{t("reports.loadingIssues")}</p>}

          {!issuesLoading && attendanceIssues.length === 0 && !issuesError && (
            <p className="text-sm text-gray-600">{t("reports.noIssues")}</p>
          )}
            ...
            <Card className="border-orange-200 bg-orange-50/50">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-gray-800">
                  ...
                  {t("reports.issuesTitle")}
                </CardTitle>
                <CardDescription>
                  {t("reports.issuesDesc")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex items-center gap-2 min-w-[120px]">
                          ...
                          <span className="text-sm text-gray-600">
                            {new Date(issue.date).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                            })}
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-600">
                          ...
                          {issue.workedHours !== null && (
                            <span className="font-medium text-yellow-700">
                              {t("reports.workedHours").replace("{hours}", String(issue.workedHours))}
                            </span>
                          )}
                        </div>
                      </div>
                ...
                {attendanceIssues.length > 10 && (
                  <div className="mt-4 text-center">
                    <Button variant="ghost" className="text-gray-600">
                      {t("reports.viewAll").replace("{count}", String(attendanceIssues.length))}
                    </Button>
                  </div>
                )}
              </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("reports.detailsDesc")}</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{t("reports.table.rate")}</TableHead>
                      <TableHead>{t("reports.table.attended")}</TableHead>
                      <TableHead>{t("reports.table.absent")}</TableHead>
                      <TableHead>{t("reports.table.totalHours")}</TableHead>
                      <TableHead>{t("reports.table.lateHours")}</TableHead>
                      <TableHead>{t("reports.table.earlyLeave")}</TableHead>
                      <TableHead>{t("reports.table.overtime")}</TableHead>
