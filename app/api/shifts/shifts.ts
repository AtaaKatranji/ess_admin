...
export const fetchTimeShifts = async (employeeId: string) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn("Failed to fetch shifts", response.status);
      return null;
    }
...
export const updateShift = async (newShift: Shift, ourSlug: string) => {
    ...
    try {
      ...
      const shiftResponse = await fetch(`${BaseUrl}/institutions/${ourSlug}/shifts/${newShift.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: "include",
